...
export default function ResultsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const candidates = calculateRecommendations(
    participants, timeBlocks, group.start_date, group.end_date, group.grid_start_hour, group.grid_end_hour
  );
  ...
  const isHost = myParticipantId && participants.find((p) => p.id === myParticipantId)?.role === 'host';
    ...
    <main style={{ minHeight: '100dvh', padding: '24px 40px', maxWidth: 800, margin: '0 auto', width: '100%', boxSizing: 'border-box' }}>
      ...
      {confirmModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: '#fff', borderRadius: 20, padding: '40px 48px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, minWidth: 260 }}>
            {confirmModal === 'loading' ? (
              <>
                <div style={{ width: 48, height: 48, border: '3px solid #eee', borderTop: '3px solid #111', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
                <p style={{ fontSize: 15, fontWeight: 500, margin: 0 }}>캘린더에 추가하는 중...</p>
              </>
            ) : (
              <>
                <div style={{ width: 52, height: 52, borderRadius: '50%', background: '#111', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg width="26" height="26" viewBox="0 0 24 24" fill="none">
                    <path d="M5 13l4 4L19 7" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                </div>
                <p style={{ fontSize: 15, fontWeight: 500, margin: 0 }}>캘린더에 추가했습니다!</p>
                <button onClick={() => setConfirmModal(null)} style={{ marginTop: 4, padding: '10px 28px', borderRadius: 10, background: '#f7f7f5', border: 'none', fontSize: 14, color: '#666' }}>
                  닫기
                </button>
              </>
            )}
...
function CandidateCard({ candidate, rank, gridStartHour, selected, onSelect }: {
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 12,
        background: '#fff',
        border: selected ? '2px solid #185fa5' : '1px solid #eee',
        cursor: 'pointer',
        display: 'flex',
        alignItems: 'center',
        gap: 12,
    ...
    >
      ...
      <div style={{
        ...
        border: selected ? '5px solid #185fa5' : '2px solid #ccc',
        background: '#fff',
      ...
      <span style={{ fontSize: 11, color: isTop ? '#0c447c' : '#999', background: isTop ? '#e6f1fb' : 'transparent', padding: isTop ? '3px 10px' : '3px 4px', borderRadius: 6, flexShrink: 0 }}>
        {rank}순위
      </span>
    </div>
  );
